...
class BiRNN(Layer):
    ...
    def forward(self, input):
        ...
        #preprocess inputs
        reverse = self._reverse_temporal_data(input, mask)

        #forward pass
        fwd_pass = self.forward_rnn.forward(input)
        bwd_pass = self.backward_rnn.forward(reverse)
        
        #reverse bwd_pass
        bwd_pass_reverse = self._reverse_temporal_data(bwd_pass, mask)

        output = np.concatenate((fwd_pass, bwd_pass_reverse), axis=2)
        #####################################################################################
